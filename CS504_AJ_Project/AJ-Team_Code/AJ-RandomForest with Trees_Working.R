...
bostAll=rpart(Manufacturing~. ,data=QI, control = list(minsplit = 10, maxdepth = 12, xval = 10)) 
...
bostAll=rpart(Manufacturing~. ,data=QI2) 
rpart.plot(bostAll, faclen=12, extra=1, digits=3, main=" Regression Model \n Manufacturing 20YRS")

bostAll=rpart(~. ,data=QI) 
rpart.plot(bostAll, faclen=12, extra=1, digits=3, main=" Regression Model \n Trade xxxxx")
bostAll=rpart(~. ,data=QI) 
rpart.plot(bostAll, faclen=12, extra=1, digits=3, main=" Regression Model \n xxxxx")
...
set.seed(123)
# Boost Training_mrty=5,6 using Manufacturing- Note Train, Train2 and Test, Test2 are available too
# cycle through
...
## together generate tree or their averages with 11 predictors 
## default ntree = 500
bag.QI=randomForest(Mining_Logging~. , data = QI, subset = train5, mtry = 5, importance = TRUE)
bag.QI
...
## From ISLR - Gradient Boost using only 6 predictors with default ntree=5000

boost.QI=gbm(medv~.,data=QI[trainbst,], distribution = "gaussian", 
...
par(mfrow = c(1, 3))
plot(yhat.bag, QI.test5)
abline(0,1, col='blue')
plot(yhat.rf, QI.test6 )
abline(0,1, col='red')
plot(yhat.boost, QI.testbst )
