...
# Modified from STAT515 D. Carr Code
########################
...
########################
# 
rfprep_Quits <- rf_prep(x=QI2[1-12], y=as.factor(QI2[, 4]), seed=2457)
rfprep_Quits$rf
rfQuits <- rf_viz(rfprep_Quits)
#
#######################
fit1<- randomForest(x = QI[,-11], y=QI[,3])
fit2<- randomForest(Leisure_Hosp ~.,data = QI2)
visreg(fit2)
fit1
####################

viz=randomForestExplainer(fit1)
###################
fit3<-randomForest(x = QI[ , 1:11], y=QI[, 3],
             importance=TRUE,  proximity=FALSE, ntree=500,
             keepForest=TRUE)
forest <- randomForest::randomForest(Manufacturing ~.,data = QI)
forest
fit3
pi=measure_importance(fit3)
ggimp<-plot_importance_ggpairs(
   pi,
   measures = NULL,
...
forest <- randomForest::randomForest(Manufacturing~.,data = QI, mtry=6,  
                                     importance=TRUE,  proximity=FALSE, ntree=1000,
                                     localimp=TRUE, keepForest=TRUE)
forest
min_depth_frame <- min_depth_distribution(forest)
head(min_depth_frame, n = 10)
plot_min_depth_distribution(min_depth_frame)

plot_predict_interaction(forest, QI, "Trade_Trans_Util", "Leisure_Hosp", main=" Prediction of the Forest for Different Values
 Trade Transportation and Utilites with Leisure and Hospitality Test")
Sforest_ranger <- ranger::ranger(Trade_Trans_Util ~., data = QI)
plot_predict_interaction(forest, QI, "Manufacturing", "Leisure_Hosp")
plot_predict_interaction(forest, QI, "Manufacturing", "Ed_Health")
plot_predict_interaction(forest, QI, "Trade_Trans_Util", "Mining_Logging",main=" Prediction of the Forest for Different Values
 Trade Transportation and Utilites with Finance")
plot_predict_interaction(forest, QI, "Trade_Trans_Util", "Manufacturing", main=" Prediction of the Forest for Different Values
 Trade Transportation and Utilites with Manufacturing Test")
plot_predict_interaction(forest, QI, "Leisure_Hosp", "Ed_Health", main="Prediction of the Forest for Different Values
Leisure and Hospitality with Education and Healthcare Test")

forest <- randomForest::randomForest(Manufacturing~.,data = QI, ntree=1000, localImp=TRUE)
save(forest, file = "QI_Forest.rda")
load("QI_Forest.rda")
explain_forest(forest, interactions = TRUE, QI)
###################
...
windows(width=9, height=9) ## For MS Windows
bostAll=rpart(Manufacturing~. ,data=QI, method = "anova", control = list(minsplit = 10, maxdepth = 12, xval = 10)) 
rpart.plot(bostAll, faclen=12, extra=1, digits=3, main=" Regression Model \n Manufacturing")
set.seed(2457)
bostAll$cptable

bostAll=rpart(Trade_Trans_Util~. ,data=QI) 
rpart.plot(bostAll, faclen=12, extra=1, digits=3, main=" Regression Model \n Trade Transportation Utilities Data")

windows(width=9, height=9) ## For MS Windows

bostAll=rpart(Manufacturing~. ,data=QI, cp=0) 
rpart.plot(bostAll, main=" Regression Model \n Manufacturing")

bostAll=rpart(Leisure_Hosp~. ,data=QI) 
rpart.plot(bostAll, faclen=12, extra=1, digits=3, main=" Regression Model \n Leisure Data")

#QI2<- read.csv(file="Quit_2016_2021q.csv", header = TRUE, 
#                   sep=',', row.names =1)
#head(QI2)
#windows(width=9, height=9) ## For MS Windows
...
set.seed(137) # also try seed of 2357
...
## default ntree = 500
bag.QI=randomForest(Manufacturing~. , data = QI, subset = train5, mtry = 10, importance = TRUE)
bag.QI
